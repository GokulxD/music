...
client.on('connected', () => {
    console.log(`Yo ${client.user.username}`);
})
...
client.on('messageCreate', async message => {  
    ...
    if(['play', 'p'].includes(commandName)) {
        ...
        if (!result) return message.chat.sendMessage(`Envy "${args.join(' ')}".`);

        const videosFiltered = result.filter(v => v.duration < 60000);
        if(!videosFiltered.length) return message.chat.sendMessage(`Error`);

        await message.chat.sendMessage(`tent ${videosFiltered.length} pour choisir la vidéo.`);
        ...
        collector.on('message', msg => {
            ...
            if(!int || int <=0 || int > videosFiltered.length) return message.chat.sendMessage('Commander');
